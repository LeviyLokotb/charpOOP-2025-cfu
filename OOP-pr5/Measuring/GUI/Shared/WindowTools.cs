...
public struct WindowTools
{
    ...
    public static Box AddPrettyBox(Orientation orientation = Orientation.Vertical, bool homogeneus = false, int margins=20, int spaces=20)
    {
        Box box = new()
        {
        ...
        box.SetOrientation(orientation);
        box.SetMarginTop(margins);
        box.SetMarginBottom(margins);
        box.SetMarginStart(margins);
        box.SetMarginEnd(margins);
        box.SetSpacing(spaces);
        return box;
    }
    /// <summary>
    ...
    /// <summary>Создаёт поле выбора из заданного массива строк</summary>
    /// <param name="fields">Варианты выбора</param>
    /// <param name="default_">Индекс варианта по-умолчанию</param>
    /// <returns>Созданный ComboBoxText</returns>
    public static ComboBoxText AddComboBox(string[] fields, int default_ = 0)
    {
        ComboBoxText combo = new();
        foreach(string s in fields) combo.AppendText(s);
        combo.SetActive(default_);
        return combo;
    }

    /// <summary>Создаёт поле выбора из заданного массива строк и добавляет в box</summary>
    /// <param name="fields"></param>
    /// <param name="box"></param>
    /// <param name="default_"></param>
    public static void AddComboBox(string[] fields, Box box, int default_ = 0) => box.Append(AddComboBox(fields, default_));
}
